...
import { trackInitiateCheckout, trackLead } from '../src/utils/tracking';
...
const Navbar: React.FC = () => {
  ...
  const handleCheckoutClick = () => {
    ...
    trackLead('Navbar Checkout Click');
  };

  useEffect(() => {
    const handleScroll = () => {
      // Throttling basic check
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-black/95 backdrop-blur-md border-b border-gray-800 py-2' : 'bg-transparent py-4'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          <div className="flex items-center cursor-pointer" onClick={() => scrollToSection('hero')}>
            {/* Logo Image with explicit dimensions for CLS */}
            <img 
              src="https://i.imgur.com/ts8zzpN.png" 
              alt="Logo ZeroCaloria - Método 30 Dias" 
              width="150"
              height="48"
        ...
        <div className="md:hidden absolute top-full left-0 w-full bg-neutral-900 border-b border-gray-800 shadow-xl">
          <div className="px-4 pt-2 pb-6 space-y-2 flex flex-col items-center">
            ...
            <a 
              href={LINKS.checkout} 
              onClick={handleCheckoutClick}
              className="mt-4 w-full block bg-yellow-500 text-black font-bold py-4 rounded-lg text-center"
            >
